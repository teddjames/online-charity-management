...
from app.utils.decorators import ngo_required, donor_required # Import the decorators
...
def update_my_profile():
    ...
    if user_role == 'NGO':
        ...
        if not ngo_profile:
            ngo_profile = NGOProfile(user_id=user_id, organization_name=data.get('organization_name', ''))
        ...
        try:
            ...
            updated_ngo_profile = ngo_profile_schema.load(ngo_profile_data, instance=ngo_profile, partial=True)
            for key, value in updated_ngo_profile.items():
        ...
        if not donor_profile:
            donor_profile = DonorProfile(user_id=user_id, first_name=data.get('first_name', ''), last_name=data.get('last_name', ''))
            db.session.add(donor_profile)

        # Load and validate Donor profile data using schema
        donor_profile_data = data.get('donor_profile', {})
        try:
            updated_donor_profile = donor_profile_schema.load(donor_profile_data, instance=donor_profile, partial=True)
            for key, value in updated_donor_profile.items():
